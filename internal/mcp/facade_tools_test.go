...
func TestCodingAgentInstructionsStayTerseAndDirective(t *testing.T) {
	...
	for _, directive := range []string{"MUST use Gortex MCP", "Start every task with explore", "Files/symbols/evidence/where", "For changes, at most one follow-up call on one unresolved symbol", `change(operation:"impact")`, "Mutate only with edit or refactor", `change(operation:"detect")`, "Call capabilities only for unknown fields"} {
