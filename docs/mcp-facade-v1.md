# MCP facade v1 specification

| Field | Value |
| --- | --- |
| Status | Implemented |
| Surface version | `facade-v1` |
...
Gortex exposes a broad code-intelligence API, but its flat MCP namespace makes an agent choose among many tools that differ only by operation or target. This specification introduces a small, stable set of **effect-homogeneous facade tools** over the existing handlers.

The facade is a compatibility layer, not a rewrite. Existing handlers remain the implementation substrate, existing CLI and HTTP entry points keep their contracts, and legacy MCP names remain available during migration. Known coding-agent clients receive a complete eager coding loop without depending on deferred-tool promotion or `notifications/tools/list_changed`.

The core workflow becomes:

```text
explore -> search / read / relations -> analyze / change
        -> edit / refactor -> change(operation="verify")
```
...
1. Reduce the initial MCP description and schema cost for coding agents.
2. Remove unnecessary agent choices such as `get_symbol` versus `get_symbol_source`, or `edit_file` versus `edit_symbol`.
...
7. Make the effective per-session surface observable and testable, especially for Codex.
...
- Replace the CLI or HTTP API with the facade naming scheme.
...
The agent preset has a 20-tool floor, plus the always-kept discovery and introspection tools. Even that curated surface contains several decisions that are implementation details rather than user intent. The full descriptions also repeat the same server guidance for each tool; in one audited Codex rendering, the repeated common prefix accounted for approximately 66% of the description characters.

The audit also exposed a discovery failure mode. The server-side preset contained `read_file`, while a Codex session did not expose a callable `read_file`. `tool_profile` reported global live state rather than the exact client-visible session surface. Facade v1 reduces the number of opportunities for this mismatch, but it also requires a direct protocol fix and regression tests; consolidation alone is not sufficient.

## 5. Design principles

### 5.1 Consolidate by intent

An agent should select a domain verb and an operation, not choose among handler names. For example, `read(operation="source", target={symbol:"Server.addTool"})` replaces a direct `get_symbol_source` selection.
...
Facade v1 defines 21 public tools. The effect is a property of the tool, not merely of an individual operation.
...
| `read` | read | Read a target through a selected legacy view | `file`, `source`, `symbol`, `symbols`, `summary`, `editing_context`, `history`, `artifact` |
...
| `trace` | read | Advanced traversal, execution, control, and data flow | `call_chain`, `walk`, `path`, `graph`, `flow`, `taint`, `cfg`, `cursor` |
| `analyze` | read | Deterministic graph analysis | `graph`, `contracts`, `architecture`, `inspections`, `communities`, `processes`, and other registered kinds |
...
| `remember` | local write | Store or update durable agent knowledge | `memory`, `note`, `edit_memory`, `rename_memory`, `notebook`, `notebook_used`, `suppress_finding` |
...
| `workspace_admin` | control write | Mutate durable repository, project, scope, index, enrichment, feedback, or persisted analysis state | `track`, `untrack`, `index`, `reindex`, `set_active_project`, `save_scope`, `delete_scope`, `enrich_churn`, `enrich_releases`, `temporal_verify`, `feedback` |
...
| `session` | session write | Mutate connection-lifetime planning, workflow, proxy, agent-coordination, or notification state | `planning_mode`, `workflow`, `proxy_enable`, `proxy_disable`, `agents`, `subscribe`, `unsubscribe` |
| `response` | read | Re-cut or export buffered tool responses | `stats`, `grep`, `slice`, `peek`, `export_context` |
| `capabilities` | read | List facade domains/operations or return an operation schema | `detail=summary`, `detail=schema` |
...
- Read operations MUST NOT persist semantic state. Mutations route through `local_write`, `session_write`, `control_write`, or `external_write` facades.
- `temporal_verify` is an explicit exception to the usual analysis placement: it invokes an LLM and persists cache/edge verdict state, so facade-v1 MUST reject it under `analyze` and expose it only as `workspace_admin(operation="temporal_verify")`.
- Forge reads MUST route through `pr`; forge writes MUST route through `publish_review`.
- Generation and graph export operations route through the local-write `edit` facade because their legacy handlers can write output.
...
- Proxy reads use `workspace`; agent registry operations and proxy/planning/workflow changes use `session`. Listing or comparing overlays uses `change`; mutating overlays uses `overlay`.
...
Codex defaults to the `facade-v1` preset in `hide` mode unless an explicit `GORTEX_TOOLS`/`--tools` selection overrides it. Its first `tools/list` advertises exactly all 21 facade names and no legacy tool names. Direct calls to hidden legacy tools are hard-blocked; facade dispatch reaches their captured handlers internally.
...
Other clients retain the existing client-aware presets. An operator can opt any connection into this contract with `GORTEX_TOOLS=facade-v1`, or explicitly select a legacy preset such as `agent`, `core`, or `full` for Codex rollback and compatibility.
...
`operation` is a normalized string rather than an ever-growing schema enum. The facade description lists common operations, while `capabilities(domain=<facade>, operation=<operation>, detail="schema")` returns the exact captured legacy input schema. The dispatcher MUST reject unknown values and return the valid operation names.
...
Cold domain facades accept an `arguments` object; common facades may additionally accept `options`, `source`, or `context`. Repository/project/scope fields use the operation schema returned by `capabilities` and are passed through to the handler. `output` centralizes the advertised `format`, `max_bytes`, `limit`, `cursor`, and `fields` controls. Cursors remain opaque.
...
The result is the existing `get_symbol_source` payload, including the metadata that handler already returns. Use `operation="symbol"` when only the legacy `get_symbol` location/signature view is wanted.
...
The `change` facade applies equally to mutations made through `edit`, `refactor`, `apply_patch`, an IDE, or another tool. Use `capabilities` to obtain the exact `verify`, `guards`, `tests`, or `contract` operation schema before a more specialized post-change check.

## 9. Capability discovery

`capabilities` replaces facade-client dependence on `tool_profile` and `tools_search`.
...
Fetch the exact captured handler schema for reading a file:
...
The result includes `surface_version`, `operation`, `effect`, `available`, and `summary`; schema detail additionally includes `input_schema` and its `schema_hash`. Domain-list responses contain the same operation records. Availability reflects whether the optional legacy handler was registered in this server configuration.
...
Fixed arguments are applied after caller arguments and cannot be overridden. Facade-v1 uses this for the overlay effect splits (`change.simulate`, `overlay.simulate`, `overlay.merge`, and `edit.apply_overlay`) and for `workspace_admin(operation="temporal_verify")` (`kind=temporal_verify`). Planning, mutation, and federation tests MUST remain in parity with the registry effects.

## 11. Compatibility and versioning

### 11.1 Surface selection

The server exposes facade-v1 as a tool preset alongside the existing legacy presets:

| Selection | Advertised tools | Intended use |
| --- | --- | --- |
| `facade-v1` (`facade`, `agent-v2`) | Exactly 21 facade tools in `hide` mode for the Codex default | Lean static contract |
| `agent`, `core`, `full`, and specialist presets | Existing legacy tool definitions and behavior | Compatibility, other clients, and explicit Codex rollback |

Codex selects `facade-v1`/`hide` automatically when no explicit surface is forwarded. `GORTEX_TOOLS` or `--tools` has higher precedence, so `GORTEX_TOOLS=full` (or another legacy preset) deliberately restores that connection's legacy surface.
...
- In `facade-v1`, the same name advertises its compact facade definition and routes through the operation registry (`analyze` retains `kind` as its primary discriminator).
...
- Introducing facade-v1 is additive.
...
- Facade-v1 `hide` mode advertises no legacy names and hard-blocks direct legacy calls. Its dispatchers call the captured legacy handlers internally without promoting or exposing their schemas.
...
## 12. Codex discovery contract

The Codex integration is correct only when the server, stdio proxy, client bridge, and session introspection agree on the visible surface.

The following sequence is normative:

```text
initialize(clientInfo.name = "codex")
  -> resolve facade-v1 / hide policy for this session
  -> tools/list returns exactly 21 static facade names and no legacy names
  -> read is present
...
1. A raw protocol test MUST initialize with `clientInfo.name="codex"` and assert the exact tool roster.
2. The test MUST assert exactly 21 facade names, `read` included, and zero legacy-name leakage.
3. `read(operation="file", target={file:"..."})` MUST return source without daemon CLI fallback.
...
8. An integration harness SHOULD compare the raw MCP roster with the functions exposed by the Codex bridge. If the bridge drops a tool, telemetry and diagnostics MUST identify the missing facade rather than recommend an uncallable name.
...
| Hook effectiveness | Hook event from an allow-list, emitted-context flag/rate bucket, daemon reachability, alternation-segment-count bucket, latency bucket |
...
- Deferred-promotion attempts in facade-v1 sessions; the expected value is zero for ordinary coding.
- Shell/raw-read fallback rate when a Gortex read facade is available.
- Mutation followed by change verification.
...
- Add raw `initialize -> tools/list` protocol tests for Codex and generic clients.
...
### Phase 3: add facade-v1 profiles

- Publish the 21-tool facade-v1 surface with the 11-tool coding core protected in restricted coding profiles.
...
### Phase 4: make Codex the hide-mode canary

- Default Codex to facade-v1 in `hide` mode while preserving explicit `GORTEX_TOOLS` rollback.
...
### Phase 5: consider other agent defaults

- Switch additional known agents only after all acceptance gates pass.
...
- A default Codex initialization resolves `facade-v1` in `hide` mode and publishes exactly the 21 facade tools defined in this specification and no legacy tools.
...
- A known file is readable with `read(operation="file", target={file:"..."})`; known symbol source is readable with `read(operation="source", target={symbol:"..."})`.
...
### 15.4 Codex correctness

- `initialize(clientInfo.name="codex") -> tools/list` returns the exact expected facade roster.
...
- Applying external mutations such as `apply_patch` can be followed by the same `change(operation="verify")` pipeline.
...
- Hook effectiveness telemetry exposes emitted-context rate, daemon reachability, alternation count, and latency by allow-listed event.
...
| Legacy tool(s) | Facade-v1 destination | Notes |
| --- | --- | --- |
...
| `get_artifact`, `get_editing_context`, `read_file`, `get_symbol_history`, `get_symbol_source`, `get_file_summary`, `get_symbol`, `batch_symbols` | `read` | Operations: `artifact`, `editing_context`, `file`, `history`, `source`, `summary`, `symbol`, `symbols` |
...
| `get_call_chain`, `get_cfg`, `nav`, `flow_between`, `graph_query`, `trace_path`, `taint_paths`, `walk_graph` | `trace` | Operations: `call_chain`, `cfg`, `cursor`, `flow`, `graph`, `path`, `taint`, `walk` |
| `audit_agent_config`, `get_architecture`, `verify_citation`, `find_clones`, `find_co_changing_symbols`, `get_communities`, `contracts`, `get_coupling_metrics`, `get_extraction_candidates`, `analyze`, `audit_health`, `run_inspections`, `list_inspections`, `get_knowledge_gaps`, `lint_file`, `get_processes`, `get_recent_changes`, `replay_episode`, `get_surprising_connections`, `get_untested_symbols`, `why`, `get_churn_rate` | `analyze` | Read operations including `graph`, architecture, quality, history, inspection, and contract analysis |
| `ask` | `ask(operation="research")` | Optional open-world research handler |
| `api_impact`, `get_code_actions`, `compare_branches`, `compare_with_overlay`, `change_contract`, `detect_changes`, `get_diagnostics`, `get_edit_plan`, `check_guards`, `explain_change_impact`, `overlay_branches`, `overlay_list`, `suggest_pattern`, `preview_edit`, `symbols_for_ranges`, `get_test_targets`, `verify_change`, `simulate_chain` | `change` | Read operations; `simulate` fixes `keep=false` so it cannot persist an overlay |
...
| `edit_memory`, `store_memory`, `save_note`, `notebook_save`, `notebook_used`, `rename_memory`, `suppress_finding` | `remember` | Local-write operations for durable agent knowledge |
...
| `analyze(kind="temporal_verify")` | `workspace_admin(operation="temporal_verify")` | Fixed `kind=temporal_verify`; facade-v1 rejects it under read-only `analyze` because it persists cache/edge verdict state |
...
| `agent_registry`, `set_planning_mode`, `workflow`, `proxy_enable`, `proxy_disable`, `subscribe_daemon_health`, `unsubscribe_daemon_health`, `subscribe_diagnostics`, `unsubscribe_diagnostics`, `subscribe_graph_invalidated`, `unsubscribe_graph_invalidated`, `subscribe_stale_refs`, `unsubscribe_stale_refs`, `subscribe_workspace_readiness`, `unsubscribe_workspace_readiness` | `session` | Session-only state. Notification calls use `operation="subscribe"|"unsubscribe"` plus `channel`, for example `channel="diagnostics"` |
...
| `tool_profile`, `tools_search` | `capabilities` compatibility mappings | Recorded as `legacy_profile` and `legacy_search`; facade-v1 clients use `domain`/`operation`/`detail` without promotion |
...
| `1.0.0-draft.2` | 2026-07-11 | Aligned selectors, operation calls, capability discovery, response metadata, and Codex hide-mode behavior with the implementation |
| `1.0.0-draft.1` | 2026-07-11 | Initial facade-v1 design based on the 178-tool audit and Codex discovery investigation |
